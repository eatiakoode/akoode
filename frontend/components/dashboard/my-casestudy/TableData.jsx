...
const TableData = () => {
   const [casestudyList, setCasestudyList] = useState([]);
    ...
    const fetchCasestudyData = async () => {
      ...
      const data = await getCasestudyTableData(filter);
      setCasestudyList(data.items);
    };
    const deleteCasestudy = async (id) => {
        ...
        try {
          const data = await deleteCasestudyAPI(id); // 🔹 Call the API function
          
          toast.success(data.message);
          setCasestudyList((prevCasestudyList) => prevCasestudyList.filter((casestudy) => casestudy._id !== id));
  ...
  let tbodyContent = casestudyList?.slice(0, 10)?.map((item) => (
    <tr key={item._id}>
      <td scope="row">
      ...
      <td>
        <ul className="view_edit_delete_list mb0">
          <li
            ...
            title="Edit"
          >
            <button  onClick={() => router.push(`/akoodeadmin/edit-casestudy/${item._id}`)}>
          ...
          <li
            ...
            title="Delete"
          >
            <a href="#"  onClick={() => deleteCasestudy(item._id)}>
