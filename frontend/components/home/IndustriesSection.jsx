"use client";

export default function IndustriesSection() {
  return (
    <>
      {/*===== INDUSTRIES AREA STARTS =======*/}
      <div
        className="service5-section-area space-margin100"
        style={{
          backgroundImage: "url(images/technology.png)",
          backgroundRepeat: "no-repeat",
          backgroundPosition: "center",
          backgroundSize: "cover",
        }}
      >
        <div className="robo-image1">
          <img src="images/round-shape1.png" alt="image" className="img-fluid rotateIn" />
        </div>
        <div className="container">
          <div className="row">
            <div className="col-lg-8">
              <div className="service-header heading7 space-margin60">
                <h5>
                  <span>
                    <img src="images/sub-logo1.svg" alt="" />
                  </span>
                  Industries We Serve
                </h5>
                <div className="space18"></div>
                <h2 className="text-anime-style-3">Tailored solutions for diverse industries</h2>
              </div>
            </div>
          </div>
          <div className="industries-slider-wrapper">
            <div className="industries-slider-box">
              <div className="row">
                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/healthcare.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg" fill="#000000">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <title></title>
                          <path
                            d="M21,56V76.86A17.07,17.07,0,0,0,38,94h0A17.07,17.07,0,0,0,55,76.86V54"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></path>
                          <circle
                            cx="21"
                            cy="48"
                            r="8"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></circle>
                          <circle
                            cx="21"
                            cy="48"
                            r="8"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></circle>
                          <polygon
                            points="71.5 6 76.5 4 71.5 2 71.5 6"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></polygon>
                          <path
                            d="M36,4H31.91C29,4,26.67,7,27,10.35,29.25,30.54,38.69,54,55,54S80.75,30.54,83,10.35C83.33,7,81,4,78.09,4H74"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></path>
                          <polygon
                            points="38.5 6 33.5 4 38.5 2 38.5 6"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></polygon>
                          <circle
                            cx="21"
                            cy="48"
                            r="2"
                            fill="none"
                            stroke="#281a3b"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="4px"
                          ></circle>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Healthcare</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/ecommerce.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="#000000">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <defs>
                            <style>{`.cls-1{fill:none;stroke:#000000;stroke-linecap:round;stroke-linejoin:round;stroke-width:1.5px;}`}</style>
                          </defs>
                          <g id="ic-ecommerce-basket">
                            <path className="cls-1" d="M17.89,20.85H6.11a1.42,1.42,0,0,1-1.38-1.08L2.08,9.16a.7.7,0,0,1,.68-.87H21.24a.7.7,0,0,1,.68.87L19.27,19.77A1.42,1.42,0,0,1,17.89,20.85Z"></path>
                            <line className="cls-1" x1="9.8" y1="3.15" x2="6.9" y2="8.29"></line>
                            <line className="cls-1" x1="13.88" y1="3.15" x2="16.79" y2="8.29"></line>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Retail & E-Commerce</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/finance.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        viewBox="0 0 846.66 846.66"
                        style={{ shapeRendering: "geometricPrecision", textRendering: "geometricPrecision", imageRendering: "optimizeQuality", fillRule: "evenodd", clipRule: "evenodd" }}
                        version="1.1"
                        xmlSpace="preserve"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <defs>
                            <style type="text/css">{`.fil0 {fill:black;fill-rule:nonzero}`}</style>
                          </defs>
                          <g id="Layer_x0020_1">
                            <path
                              className="fil0"
                              d="M797.09 226.07c0,-64.22 -52.06,-116.28 -116.28,-116.28 -64.22,0 -116.28,52.06 -116.28,116.28 0,64.22 52.06,116.28 116.28,116.28 63.81,0 116.28,-51.65 116.28,-116.28zm-427.01 188.31c-27.31,0 -27.31,-41.52 0,-41.52l83.06 0c27.31,0 27.31,41.52 0,41.52l-83.06 0zm0 74.75c-27.31,0 -27.31,-41.52 0,-41.52l279.28 0c27.32,0 27.32,41.52 0,41.52l-279.28 0zm-249.17 -144.3l166.12 0c11.46,0 20.76,9.29 20.76,20.76l0 130.81c0,11.47 -9.3,20.77 -20.76,20.77l-166.12 0c-11.46,0 -20.76,-9.3 -20.76,-20.77l0 -130.81c0,-11.47 9.3,-20.76 20.76,-20.76zm145.35 41.52l-124.58 0 0 89.29 124.58 0 0 -89.29zm336.07 -152.7c-16.11,-21.98 17.31,-46.47 33.41,-24.5l18.25 24.94 75.45 -61.89c21,-17.27 47.27,14.69 26.28,31.95l-91.56 75.11c-9.1,8.28 -23.38,6.92 -30.69,-3.05l-31.14 -42.56zm-552.76 375.92l689.38 0 0 -236.75c-17.99,7.14 -37.61,11.06 -58.14,11.06 -79.13,0 -146.16,-58.73 -156.45,-137.05l-416.65 0c-31.93,0 -58.14,26.21 -58.14,58.15l0 304.59zm689.38 41.53l-689.38 0 0 27.63c0,31.94 26.21,58.14 58.14,58.14l573.1 0c31.93,0 58.14,-26.2 58.14,-58.14l0 -27.63zm41.53 -302.68l0 330.31c0,54.86 -44.81,99.67 -99.67,99.67l-573.1 0c-54.86,0 -99.67,-44.81 -99.67,-99.67l0 -373.75c0,-54.87 44.81,-99.67 99.67,-99.67l416.65 0c10.29,-78.32 77.32,-137.05 156.45,-137.05 87.15,0 157.81,70.66 157.81,157.81 0,47.63 -21.31,92.32 -58.14,122.35z"
                            ></path>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Finance & Banking</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/manufacturing.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" fill="#000000">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path fill="#444" d="M12 7v-2l-1.2-0.4c-0.1-0.3-0.2-0.7-0.4-1l0.6-1.2-1.5-1.3-1.1 0.5c-0.3-0.2-0.6-0.3-1-0.4l-0.4-1.2h-2l-0.4 1.2c-0.3 0.1-0.7 0.2-1 0.4l-1.1-0.5-1.4 1.4 0.6 1.2c-0.2 0.3-0.3 0.6-0.4 1l-1.3 0.3v2l1.2 0.4c0.1 0.3 0.2 0.7 0.4 1l-0.5 1.1 1.4 1.4 1.2-0.6c0.3 0.2 0.6 0.3 1 0.4l0.3 1.3h2l0.4-1.2c0.3-0.1 0.7-0.2 1-0.4l1.2 0.6 1.4-1.4-0.6-1.2c0.2-0.3 0.3-0.6 0.4-1l1.2-0.4zM3 6c0-1.7 1.3-3 3-3s3 1.3 3 3c0 1.7-1.3 3-3 3s-3-1.3-3-3z"></path>
                          <path fill="#444" d="M7.5 6c0 0.828-0.672 1.5-1.5 1.5s-1.5-0.672-1.5-1.5c0-0.828 0.672-1.5 1.5-1.5s1.5 0.672 1.5 1.5z"></path>
                          <path fill="#444" d="M16 3v-1h-0.6c0-0.2-0.1-0.4-0.2-0.5l0.4-0.4-0.7-0.7-0.4 0.4c-0.2-0.1-0.3-0.2-0.5-0.2v-0.6h-1v0.6c-0.2 0-0.4 0.1-0.5 0.2l-0.4-0.4-0.7 0.7 0.4 0.4c-0.1 0.2-0.2 0.3-0.2 0.5h-0.6v1h0.6c0 0.2 0.1 0.4 0.2 0.5l-0.4 0.4 0.7 0.7 0.4-0.4c0.2 0.1 0.3 0.2 0.5 0.2v0.6h1v-0.6c0.2 0 0.4-0.1 0.5-0.2l0.4 0.4 0.7-0.7-0.4-0.4c0.1-0.2 0.2-0.3 0.2-0.5h0.6zM13.5 3.5c-0.6 0-1-0.4-1-1s0.4-1 1-1 1 0.4 1 1c0 0.6-0.4 1-1 1z"></path>
                          <path fill="#444" d="M15.4 11.8c-0.1-0.3-0.2-0.6-0.4-0.9l0.3-0.6-0.7-0.7-0.5 0.4c-0.3-0.2-0.6-0.3-0.9-0.4l-0.2-0.6h-1l-0.2 0.6c-0.3 0.1-0.6 0.2-0.9 0.4l-0.6-0.3-0.7 0.7 0.3 0.6c-0.2 0.3-0.3 0.6-0.4 0.9l-0.5 0.1v1l0.6 0.2c0.1 0.3 0.2 0.6 0.4 0.9l-0.3 0.6 0.7 0.7 0.6-0.3c0.3 0.2 0.6 0.3 0.9 0.4l0.1 0.5h1l0.2-0.6c0.3-0.1 0.6-0.2 0.9-0.4l0.6 0.3 0.7-0.7-0.4-0.5c0.2-0.3 0.3-0.6 0.4-0.9l0.6-0.2v-1l-0.6-0.2zM12.5 14c-0.8 0-1.5-0.7-1.5-1.5s0.7-1.5 1.5-1.5 1.5 0.7 1.5 1.5-0.7 1.5-1.5 1.5z"></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Manufacturing</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/education.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        version="1.1"
                        id="_x32_"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 512 512"
                        xmlSpace="preserve"
                        fill="#000000"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <style type="text/css">{`.st0{fill:#000000;}`}</style>
                          <g>
                            <path
                              className="st0"
                              d="M108.182,117.635c32.489,0,58.817-26.332,58.817-58.817C166.999,26.332,140.671,0,108.182,0 c-32.49,0-58.818,26.332-58.818,58.818C49.364,91.302,75.692,117.635,108.182,117.635z"
                            ></path>
                            <circle className="st0" cx="86.763" cy="372.301" r="50.668"></circle>
                            <path
                              className="st0"
                              d="M87,447.641c-35.278-0.258-64.098,28.122-64.36,63.404L150.408,512C150.67,476.718,122.282,447.908,87,447.641 z"
                            ></path>
                            <circle className="st0" cx="254.425" cy="372.301" r="50.668"></circle>
                            <path
                              className="st0"
                              d="M254.662,447.641c-35.278-0.258-64.097,28.122-64.359,63.404L318.07,512 C318.332,476.718,289.943,447.908,254.662,447.641z"
                            ></path>
                            <circle className="st0" cx="422.087" cy="372.301" r="50.668"></circle>
                            <path
                              className="st0"
                              d="M422.328,447.641c-35.283-0.258-64.102,28.122-64.368,63.404L485.732,512 C485.99,476.718,457.61,447.908,422.328,447.641z"
                            ></path>
                            <path
                              className="st0"
                              d="M197.428,228.807c0.336-45.204-32.894-82.706-76.364-89.168v22.391H94.628v-22.382 c-42.915,6.368-76.029,43.087-76.364,87.826v58.912h179.163V228.807z M121.064,224.831H94.628v-44.334h26.436V224.831z"
                            ></path>
                            <path
                              className="st0"
                              d="M301.513,93.135c-9.01,0-16.316,7.306-16.316,16.325c0,9.018,7.306,16.315,16.316,16.315 c9.018,0,16.328-7.297,16.328-16.315C317.842,100.441,310.531,93.135,301.513,93.135z"
                            ></path>
                            <path
                              className="st0"
                              d="M430.692,93.135c-9.01,0-16.325,7.306-16.325,16.325c0,9.018,7.315,16.315,16.325,16.315 c9.01,0,16.324-7.297,16.324-16.315C447.016,100.441,439.702,93.135,430.692,93.135z"
                            ></path>
                            <path
                              className="st0"
                              d="M366.101,93.135c-9.01,0-16.307,7.306-16.307,16.325c0,9.018,7.297,16.315,16.307,16.315 c9.018,0,16.324-7.297,16.324-16.315C382.425,100.441,375.119,93.135,366.101,93.135z"
                            ></path>
                            <path
                              className="st0"
                              d="M417.432,0.809H306.767c-42.072,0-76.3,34.232-76.3,76.32v29.783l-17.194,10.155L176,139.07l36.604,23.106 l28.049,17.718c13.515,23.467,38.586,38.208,66.115,38.208h110.664c42.072,0,76.304-34.232,76.304-76.312V77.129 C493.735,35.041,459.503,0.809,417.432,0.809z M467.3,141.79c0,27.546-22.331,49.877-49.868,49.877H306.767 c-20.572,0-38.216-12.47-45.841-30.24l-34.21-21.599l30.188-17.822V77.129c0-27.554,22.327-49.885,49.864-49.885h110.664 c27.537,0,49.868,22.331,49.868,49.885V141.79z"
                            ></path>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Education & E-Learning</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/logistics.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        version="1.1"
                        id="Capa_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 612 612"
                        xmlSpace="preserve"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <g>
                            <path d="M391.897,259.865c-0.737-6.115-3.061-11.692-6.633-16.349l24.524-152.198c0.913-5.663-2.72-11.062-8.308-12.353 l-0.274-0.062c-5.588-1.291-11.219,1.971-12.882,7.46l-44.676,147.543c-11.01,5.536-18.653,16.821-18.653,29.964 c0,18.558,15.091,33.658,33.648,33.658c3.458,0,6.728-0.675,9.869-1.647l93.334,48.503c5.135,2.668,11.456,1.002,14.607-3.852 l0.141-0.217c3.152-4.852,2.104-11.305-2.424-14.91L391.897,259.865z M342.023,263.87c0-9.163,7.458-16.621,16.621-16.621 c9.171,0,16.629,7.458,16.629,16.621c0,9.17-7.458,16.629-16.629,16.629C349.481,280.499,342.023,273.041,342.023,263.87z M612,263.87c0,112.658-73.93,208.333-175.809,241.169l-0.005-36.076c82.756-31.396,141.757-111.467,141.757-205.093 c0-120.92-98.38-219.299-219.299-219.299c-120.92,0-219.299,98.379-219.299,219.299c0,12.13,1.034,24.019,2.938,35.62h-34.441 c-1.646-11.646-2.554-23.527-2.554-35.62c0-139.703,113.653-253.356,253.356-253.356C498.347,10.514,612,124.167,612,263.87z M402.81,444.398l-77.99-77.139c-4.938-4.939-11.664-7.664-18.561-7.664h-39.336v-14.729c0-10.984-8.939-19.838-19.923-19.838 H19.841c-10.983,0-19.838,8.854-19.838,19.838v132.82h410.641v-14.473C410.643,456.148,407.833,449.336,402.81,444.398z M368.838,446.953h-75.521c-1.703,0-3.065-1.363-3.065-3.066v-58.322c0-1.703,1.361-3.066,3.065-3.066h14.389 c0.767,0,1.618,0.342,2.214,0.852l61.047,58.324C373.01,443.545,371.647,446.953,368.838,446.953z M410.652,488.66v43.941 c0,10.961-8.886,19.848-19.848,19.848h-21.237c-3.837-26.332-26.53-46.576-53.919-46.576c-27.325,0-50.017,20.244-53.854,46.576 H154.151c-3.837-26.332-26.53-46.576-53.854-46.576c-27.324,0-50.017,20.244-53.854,46.576H19.848 C8.886,552.447,0,543.562,0,532.602V488.66H410.652z M315.709,519.152c-22.732,0-41.208,18.391-41.208,41.123 c0,22.734,18.476,41.211,41.208,41.211c22.733,0,41.124-18.477,41.124-41.211C356.833,537.543,338.442,519.152,315.709,519.152z M315.709,580.881c-11.408,0-20.604-9.195-20.604-20.605c0-11.324,9.195-20.604,20.604-20.604c11.324,0,20.52,9.279,20.52,20.604 C336.229,571.686,327.033,580.881,315.709,580.881z M100.3,519.152c-22.732,0-41.209,18.391-41.209,41.123 c0,22.734,18.477,41.211,41.209,41.211c22.733,0,41.124-18.477,41.124-41.211C141.424,537.543,123.033,519.152,100.3,519.152z M100.3,580.881c-11.409,0-20.604-9.195-20.604-20.605c0-11.324,9.195-20.604,20.604-20.604c11.324,0,20.604,9.279,20.604,20.604 C120.904,571.686,111.624,580.881,100.3,580.881z"></path>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Logistics & Supply Chain</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/real-estate.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg viewBox="0 0 1024 1024" className="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path
                            d="M618.2 340.5H220.9l181.6-190.8 134.2 144.1c8.7 9.3 23.4 9.9 32.8 1.2 9.4-8.7 9.9-23.4 1.2-32.7l-151-162.2c-4.4-4.7-10.5-7.4-16.9-7.4-6.5 0.5-12.5 2.6-17 7.2L150 347.7c-4.1 4.3-6.4 10-6.4 16v534.8c0 12.8 10.4 23.2 23.2 23.2h451.4c12.8 0 23.2-10.4 23.2-23.2V363.7c0-12.8-10.3-23.2-23.2-23.2zM595 875.3H190V386.8h405v488.5z m262.3-440H699.5c-12.8 0-23.2 10.4-23.2 23.2 0 12.8 10.4 23.2 23.2 23.2H834v393.9H699.5c-12.8 0-23.2 10.4-23.2 23.2 0 12.8 10.4 23.2 23.2 23.2h157.7c12.8 0 23.2-10.4 23.2-23.2V458.4c0.1-12.8-10.3-23.1-23.1-23.1z m-339 21.2h-78.8v78.6h78.8v-78.6z m-172.7 0h-78.8v78.6h78.8v-78.6z m172.7 139.2h-78.8v78.6h78.8v-78.6z m-172.7 0h-78.8v78.6h78.8v-78.6z m172.7 139.1h-78.8v78.6h78.8v-78.6z m-172.7 0h-78.8v78.6h78.8v-78.6z"
                            fill="#34B97F"
                          ></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Real Estate</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/energy.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg fill="#000000" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path d="M13,9h6L8,22l3-10H5L10,2h7Z"></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Energy & Utilities</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/travel.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg fill="#000000" viewBox="-2.5 0 19 19" xmlns="http://www.w3.org/2000/svg" className="cf-icon-svg">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path d="M12.382 5.304 10.096 7.59l.006.02L11.838 14a.908.908 0 0 1-.211.794l-.573.573a.339.339 0 0 1-.566-.08l-2.348-4.25-.745-.746-1.97 1.97a3.311 3.311 0 0 1-.75.504l.44 1.447a.875.875 0 0 1-.199.79l-.175.176a.477.477 0 0 1-.672 0l-1.04-1.039-.018-.02-.788-.786-.02-.02-1.038-1.039a.477.477 0 0 1 0-.672l.176-.176a.875.875 0 0 1 .79-.197l1.447.438a3.322 3.322 0 0 1 .504-.75l1.97-1.97-.746-.744-4.25-2.348a.339.339 0 0 1-.08-.566l.573-.573a.909.909 0 0 1 .794-.211l6.39 1.736.02.006 2.286-2.286c.37-.372 1.621-1.02 1.993-.65.37.372-.279 1.622-.65 1.993z"></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Travel & Hospitality</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/media.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg fill="#000000" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path d="M496.64 496.644c-11.311 0-20.48-9.169-20.48-20.48v-112.64c0-11.311 9.169-20.48 20.48-20.48h112.64c11.311 0 20.48 9.169 20.48 20.48v112.64c0 11.311-9.169 20.48-20.48 20.48H496.64zm92.16-40.96v-71.68h-71.68v71.68h71.68zm252.357-42.296c-2.978-6.862-8.649-12.533-15.511-15.511-32.717-14.187-63.077 16.173-48.892 48.884 2.98 6.868 8.651 12.539 15.514 15.517 32.717 14.187 63.077-16.173 48.89-48.89zm37.577-16.3c28.967 66.801-35.966 131.734-102.768 102.768-16.398-7.116-29.671-20.389-36.789-36.792-28.964-66.796 35.969-131.729 102.77-102.762 16.398 7.116 29.671 20.389 36.787 36.787zM710.287 113.25c16.001-5.367 31.261 9.837 25.954 25.857l-47.79 144.261c-4.802 14.495-23.205 18.827-33.968 7.996l-95.57-96.174c-10.7-10.767-6.377-29.025 8.014-33.853l143.36-48.087zm-99.093 76.441l48.854 49.163 24.43-73.745-73.284 24.582zm-231.962-73.357c60.974 65.665 60.974 172.704 0 238.369-7.696 8.289-7.216 21.247 1.072 28.943s21.247 7.216 28.943-1.072c75.566-81.381 75.566-212.73 0-294.111-7.696-8.289-20.655-8.769-28.943-1.072s-8.769 20.655-1.072 28.943zm300.74 550.054c65.551-64.574 171.944-64.574 237.495 0 8.058 7.938 21.025 7.84 28.962-.217s7.84-21.025-.217-28.962c-81.496-80.281-213.489-80.281-294.985 0-8.058 7.938-8.155 20.905-.217 28.962s20.905 8.155 28.962.217zM75.705 905.107L269.64 446.283c12.228-28.951 44.791-35.314 67.027-13.098l254.151 254.141c22.234 22.215 15.876 54.784-13.077 67.032l-458.853 193.91c-32.495 13.755-56.91-10.659-43.182-43.161zM561.79 716.631c1.622-.686 1.643-.709 1.484.106-.16.821-.167.801-1.412-.442L307.711 462.154c-1.242-1.241-1.268-1.251-.452-1.41s.798-.142.112 1.48l-186.245 440.63L561.79 716.631zM121.126 902.855l-18.194 7.689c-1.461.618-2.22.799-2.018.799 2.335.006 5.371 1.087 8.007 3.723 2.635 2.635 3.715 5.665 3.721 7.996.001.198.181-.564.794-2.016l7.689-18.191z"></path>
                          <path d="M175.642 749.561l98.796 98.785c7.998 7.998 20.966 7.997 28.963-.002s7.997-20.966-.002-28.963l-98.796-98.785c-7.998-7.998-20.966-7.997-28.963.002s-7.997 20.966.002 28.963z"></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Media & Entertainment</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/automotive.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        version="1.1"
                        id="Capa_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 357.25 357.25"
                        xmlSpace="preserve"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path d="M127.712,283.641c-40.053-19.423-65.933-60.717-65.933-105.201c0-39.34,19.617-75.8,52.475-97.531 c4.146-2.742,5.284-8.326,2.542-12.472s-8.325-5.284-12.472-2.542c-1.855,1.228-3.673,2.494-5.455,3.801L81.093,54.779 c-10.266-8.613-25.627-7.27-34.241,2.996L36.731,69.839c-4.164,4.961-6.145,11.261-5.579,17.74 c0.567,6.48,3.612,12.341,8.573,16.502l17.807,14.943c-4.276,8.717-7.613,17.911-9.927,27.423H24.304 C10.903,146.447,0,157.35,0,170.751v15.75c0,13.401,10.903,24.304,24.304,24.304h23.395c2.323,9.426,5.662,18.577,9.973,27.305 l-17.944,15.059c-4.963,4.163-8.008,10.023-8.575,16.504c-0.566,6.479,1.415,12.779,5.577,17.737l10.123,12.068 c4.801,5.72,11.696,8.668,18.638,8.668c5.514,0,11.059-1.862,15.604-5.674l18.01-15.113c6.503,4.752,13.445,8.933,20.753,12.477 c4.473,2.167,9.856,0.301,12.025-4.171C134.052,291.193,132.184,285.809,127.712,283.641z M24.304,192.806 c-3.476,0-6.304-2.828-6.304-6.304v-15.75c0-3.476,2.828-6.304,6.304-6.304H44.5c-0.478,4.618-0.721,9.287-0.721,13.992 c0,4.82,0.255,9.614,0.76,14.366H24.304z M51.294,90.29c-1.277-1.071-2.062-2.591-2.21-4.279c-0.147-1.689,0.362-3.323,1.436-4.602 l10.122-12.065c1.245-1.484,3.033-2.249,4.833-2.249c1.431,0,2.869,0.483,4.048,1.472L84.898,81.47 c-6.863,6.624-12.958,13.911-18.219,21.731L51.294,90.29z M69.525,288.683c-2.665,2.235-6.651,1.886-8.884-0.774L50.519,275.84 c-1.072-1.277-1.582-2.911-1.434-4.6c0.148-1.688,0.933-3.208,2.212-4.282l15.56-13.058c5.334,7.889,11.457,15.156,18.256,21.703 L69.525,288.683z M332.946,146.447h-23.283c-2.309-9.528-5.642-18.729-9.916-27.446l17.778-14.919 c10.265-8.613,11.609-23.974,2.996-34.242l-10.123-12.065c-8.614-10.266-23.975-11.61-34.241-2.996l-17.775,14.916 c-2.349-1.721-4.766-3.376-7.243-4.959c-4.189-2.676-9.754-1.45-12.43,2.739c-2.676,4.189-1.45,9.754,2.739,12.43 c33.826,21.611,54.021,58.446,54.021,98.535c0,45.388-26.627,87.036-67.836,106.104c-4.511,2.087-6.476,7.437-4.389,11.947 c2.087,4.511,7.438,6.477,11.947,4.389c8.167-3.779,15.837-8.326,22.941-13.532l18.024,15.124c4.404,3.696,9.864,5.672,15.568,5.672 c0.721,0,1.445-0.032,2.172-0.095c6.479-0.567,12.34-3.611,16.502-8.572l10.123-12.064c8.614-10.267,7.271-25.627-2.996-34.242 l-18.023-15.125c4.278-8.657,7.641-17.785,9.994-27.24h23.449c13.401,0,24.304-10.903,24.304-24.304v-15.75 C357.25,157.35,346.347,146.447,332.946,146.447z M305.956,266.958c2.663,2.235,3.012,6.22,0.777,8.884l-10.123,12.064 c-1.072,1.277-2.592,2.062-4.281,2.209c-1.694,0.146-3.323-0.361-4.601-1.434l-15.625-13.111 c6.833-6.601,12.938-13.892,18.225-21.728L305.956,266.958z M290.6,103.18c-5.261-7.814-11.365-15.1-18.235-21.72l15.363-12.891 c2.663-2.234,6.647-1.886,8.882,0.776l10.122,12.064c2.235,2.664,1.887,6.65-0.776,8.885L290.6,103.18z M339.25,186.501 c0,3.476-2.828,6.304-6.304,6.304h-20.251c0.512-4.74,0.773-9.536,0.773-14.366c0-4.707-0.242-9.377-0.716-13.992h20.193 c3.476,0,6.304,2.828,6.304,6.304V186.501z M239.213,178.439c0-21.093-10.711-40.345-28.654-51.498 c-4.221-2.624-5.516-8.173-2.892-12.395c2.624-4.22,8.173-5.517,12.395-2.892c23.263,14.46,37.151,39.426,37.151,66.785 c0,18.602-6.619,36.647-18.636,50.813c-1.78,2.098-4.315,3.178-6.867,3.178c-2.058,0-4.126-0.702-5.818-2.137 c-3.791-3.215-4.256-8.895-1.041-12.685C234.112,206.69,239.213,192.781,239.213,178.439z M133.413,218.772 c3.311,3.708,2.988,9.397-0.72,12.708c-1.716,1.532-3.858,2.286-5.991,2.286c-2.475,0-4.939-1.015-6.717-3.006 c-12.862-14.409-19.946-32.99-19.946-52.32c0-29.089,15.956-55.672,41.641-69.376c4.385-2.339,9.837-0.682,12.177,3.704 c2.34,4.385,0.682,9.837-3.704,12.177c-19.808,10.568-32.113,31.066-32.113,53.495C118.039,193.343,123.499,207.667,133.413,218.772 z M187.474,207.429c0.1-0.538,0.152-1.092,0.152-1.659V94.305l32.654-18.853c2.785-1.608,4.5-4.579,4.5-7.794V24.754 c0-3.215-1.715-6.187-4.5-7.794L201.704,6.235c-4.304-2.485-9.81-1.011-12.294,3.294c-2.485,4.305-1.01,9.809,3.294,12.294 l14.076,8.127v32.511l-28.154,16.255L150.47,62.462V29.951l14.078-8.127c4.305-2.485,5.78-7.989,3.295-12.294 c-2.485-4.305-7.99-5.78-12.294-3.295L136.97,16.96c-2.785,1.607-4.5,4.579-4.5,7.794v42.904c0,3.215,1.715,6.187,4.5,7.794 l32.656,18.853V205.77c0,0.567,0.052,1.121,0.152,1.659c-12.179,3.78-21.048,15.152-21.048,28.557v95.4 c0,11.49,9.346,20.837,20.833,20.837h18.12c11.49,0,20.837-9.347,20.837-20.837v-95.4 C208.521,222.581,199.652,211.209,187.474,207.429z M190.521,331.386c0,1.538-1.299,2.837-2.837,2.837h-18.12 c-1.536,0-2.833-1.299-2.833-2.837v-95.4c0-6.558,5.336-11.893,11.896-11.893s11.896,5.335,11.896,11.893V331.386z"></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Automotive</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/agriculture.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        version="1.1"
                        id="Layer_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 512 512"
                        xmlSpace="preserve"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <g>
                            <g>
                              <circle cx="163.366" cy="43.617" r="43.617"></circle>
                            </g>
                          </g>
                          <g>
                            <g>
                              <path d="M250.801,367.775l-35.728-93.833l0.059-45.057l-86.587,14.509c1.812,14.118-4.31,27.914-15.345,36.144 c2.5,20.196,12.375,99.942,12.375,99.942L94.808,480.036c-3.995,13.057,3.352,26.881,16.409,30.876 c13.082,4,26.888-3.374,30.876-16.409l32.309-105.597c1.016-3.323,1.322-6.823,0.894-10.272l-9.583-77.391l6.881,0.11 l30.126,79.119l-5.726,105.462C196.228,500.055,207.456,512,221.704,512c13.04-0.001,23.952-10.207,24.668-23.385l6.011-110.7 C252.57,374.462,252.031,371.007,250.801,367.775z"></path>
                            </g>
                          </g>
                          <g>
                            <g>
                              <path d="M39.424,215.667l-10.205,1.71c-6.549,1.097-10.966,7.294-9.87,13.842c1.098,6.557,7.305,10.966,13.842,9.87l13.208-2.213 L39.424,215.667z"></path>
                            </g>
                          </g>
                          <g>
                            <g>
                              <g>
                                <path d="M236.198,162.635C236,162.58,236.03,162.588,236.198,162.635L236.198,162.635z"></path>
                                <path d="M492.651,151.914c-1.097-6.548-7.305-10.964-13.842-9.87l-128.015,21.45l-2.672-13.736 c12.874-2.909,17.955-13.96,15.79-23.535c-2.37-10.492-12.796-17.071-23.29-14.704l-0.134,0.03 c-2.054-5.337-7.681-8.6-13.501-7.471c-6.053,1.177-10.148,6.708-9.685,12.711l-55.471,12.535L201.88,112.69 c-4.666-3.287-10.249-5.209-16.254-5.217l-38.922-0.798c-8.129-0.167-15.984,2.941-21.8,8.623 c-2.168,2.119-3.984,4.529-5.411,7.14l14.166-2.632l-74.935,33.193c-8.976,3.976-13.589,14.011-10.764,23.413l23.295,77.502 c3.098,10.307,13.966,16.142,24.257,13.045c10.301-3.096,16.143-13.956,13.046-24.257l-18.348-61.047l59.227-26.234l-37.8,29.865 c12.475-2.09,90.966-15.242,103.575-17.355l0.015-11.113c17.708,4.914,23.217,6.442,24.163,6.705 c-0.591-0.164-2.642-0.733-3.192-0.885c0.818,0.227,5.475,1.519,19.937,5.532c3.101,0.86,6.359,0.939,9.501,0.23l59.021-13.337 l2.414,12.41l-210.203,35.221l6.976,23.209l316.458-53.025c15.193,42.897,1.388,107.855-6.699,138.369 c-0.335,1.267,0.311,2.588,1.517,3.101c1.206,0.512,2.605,0.06,3.285-1.06c28.871-47.608,39.559-97.47,35.191-145.989 l9.183-1.539C489.329,164.658,493.748,158.461,492.651,151.914z M239.513,163.554 C239.563,163.568,239.561,163.568,239.513,163.554L239.513,163.554z"></path>
                              </g>
                            </g>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Agriculture</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/telecommunication.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        viewBox="0 0 64 64"
                        id="Layer_1_1_"
                        version="1.1"
                        xmlSpace="preserve"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <g>
                            <rect height="2" width="2" x="61" y="61"></rect>
                            <rect height="2" width="2" x="1" y="61"></rect>
                            <path d="M32,28c1.654,0,3-1.346,3-3s-1.346-3-3-3s-3,1.346-3,3S30.346,28,32,28z M32,24c0.552,0,1,0.448,1,1s-0.448,1-1,1 s-1-0.448-1-1S31.448,24,32,24z"></path>
                            <path d="M43.697,23l9,6H55v-5.101c2.279-0.465,4-2.484,4-4.899s-1.721-4.434-4-4.899V9h-2.303l-9,6h-3.079l-2-4H33V6.816 C34.161,6.402,35,5.302,35,4c0-1.654-1.346-3-3-3s-3,1.346-3,3c0,1.302,0.839,2.402,2,2.816V11h-5.618l-2,4h-3.079l-9-6H9v5.101 C6.721,14.566,5,16.586,5,19s1.721,4.434,4,4.899V29h2.303l9-6h0.58l-3.778,34H16c-2.414,0-4.434,1.721-4.899,4H5v2h54v-2h-6.101 c-0.465-2.279-2.484-4-4.899-4h-1.105l-3.778-34H43.697z M32,3c0.552,0,1,0.448,1,1s-0.448,1-1,1s-1-0.448-1-1S31.448,3,32,3z M57,19c0,1.302-0.839,2.402-2,2.816v-5.631C56.161,16.598,57,17.698,57,19z M53,11.202v15.596l-8-5.333v-4.93L53,11.202z M26.618,13h10.764l1,2H25.618L26.618,13z M21,17h22v4h-5.264c-1.267-1.811-3.364-3-5.736-3s-4.469,1.189-5.736,3H21V17z M21.599,43L32,38.105L42.4,43H21.599z M42.401,45L32,49.894L21.599,45H42.401z M20.881,41.126l0.872-7.843L29.65,37L20.881,41.126 z M29.651,51L19.24,55.899l1.035-9.311L29.651,51z M32,52.106L42.401,57H21.599L32,52.106z M34.35,51l9.376-4.412l1.035,9.311 L34.35,51z M34.349,37l7.898-3.717l0.872,7.844L34.349,37z M42.014,31.182L32,35.894l-10.014-4.713l0.043-0.384l4.133-1.945 C27.416,30.747,29.564,32,32,32s4.584-1.253,5.838-3.147l4.133,1.945L42.014,31.182z M27,25c0-2.757,2.243-5,5-5s5,2.243,5,5 s-2.243,5-5,5S27,27.757,27,25z M7,19c0-1.302,0.839-2.402,2-2.816v5.631C7.839,21.402,7,20.302,7,19z M11,26.798V11.202l8,5.333 v4.93L11,26.798z M25.295,23C25.105,23.634,25,24.305,25,25c0,0.711,0.108,1.397,0.306,2.044l-3.018,1.42L22.895,23H25.295z M50.816,61H13.184c0.414-1.161,1.514-2,2.816-2h32C49.302,59,50.402,59.839,50.816,61z M41.712,28.464l-3.018-1.42 C38.892,26.397,39,25.711,39,25c0-0.695-0.105-1.366-0.295-2h2.4L41.712,28.464z"></path>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Telecommunication</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/public-sector.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        version="1.1"
                        id="Layer_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 256 229"
                        enableBackground="new 0 0 256 229"
                        xmlSpace="preserve"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <path d="M192.498,97.8c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8s-16.8-7.522-16.8-16.8S183.22,97.8,192.498,97.8z M171.798,139.6c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8s-16.8-7.522-16.8-16.8S162.52,139.6,171.798,139.6z M144.998,176.3h-18.9h-18.9c-11.5,0-18.7,9.5-18.7,21.4V227h12.9v-25.9c0-1.2,1-2,2-2c1.2,0,2,0.8,2,2v25.8h41.5v-25.8 c0-1.2,1-2,2-2c1.2,0,2,1,2,2v25.8h12.9v-29.1C163.998,185.8,156.698,176.3,144.998,176.3z M149.698,97.8 c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8s-16.8-7.522-16.8-16.8S140.42,97.8,149.698,97.8z M199.098,156.4 c0,9.3,7.5,16.8,16.8,16.8s16.8-7.5,16.8-16.8s-7.5-16.8-16.8-16.8S199.098,147.1,199.098,156.4z M197.398,176.3 c-11.5,0-18.7,9.5-18.7,21.4V227h12.9v-25.9c0-1.2,1-2,2-2c1.2,0,2,0.8,2,2v25.8h41.5v-25.8c0-1.2,1-2,2-2c1.2,0,2,1,2,2v25.8h12.9 v-29.1c0.2-12.1-7.1-21.6-18.7-21.6h-18.9h-19V176.3z M39.798,139.6c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8 s-16.8-7.522-16.8-16.8S30.52,139.6,39.798,139.6z M14.798,226.9v-25.8c0-1.2,1-2,2-2c1.2,0,2,0.8,2,2v25.8h41.5v-25.8 c0-1.2,1-2,2-2c1.2,0,2,1,2,2v25.8h12.9v-29.1c0.2-12.1-7.1-21.6-18.7-21.6h-18.9h-18.9c-11.5,0-18.7,9.5-18.7,21.4v29.3 L14.798,226.9L14.798,226.9z M109.298,156.4c0,9.3,7.5,16.8,16.8,16.8c9.3,0,16.8-7.5,16.8-16.8s-7.5-16.8-16.8-16.8 S109.298,147.1,109.298,156.4z M61.298,97.8c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8s-16.8-7.522-16.8-16.8 S52.02,97.8,61.298,97.8z M106.698,97.8c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8s-16.8-7.522-16.8-16.8 S97.42,97.8,106.698,97.8z M84.098,139.6c9.278,0,16.8,7.522,16.8,16.8s-7.522,16.8-16.8,16.8s-16.8-7.522-16.8-16.8 S74.82,139.6,84.098,139.6z M150.863,35.108c0,12.672-10.245,22.755-22.755,22.755c-12.51,0-22.755-10.191-22.755-22.755 c0-12.672,10.245-22.755,22.755-22.755C140.618,12.353,150.863,22.49,150.863,35.108z M196.157,2v66H59.843V2H196.157z M176.152,8.632H79.74c-1.779,6.578-6.686,11.971-13.319,13.642v25.289C73,49.343,78.069,54.735,79.74,60.235h96.52 c1.779-5.5,6.794-11,13.426-12.672V22.275C183,20.603,177.716,15.211,176.152,8.632z"></path>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Public Sector & Government</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>

                <div className="col p-0">
                  <div className="service-slider-box">
                    <img src="images/industries/insurance.jpg" alt="" className="img-fluid" />
                    <div className="shape-top">
                      <img src="images/industries/offter-item-shape-top.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="shape-bottom">
                      <img src="images/industries/offter-item-shape-bottom.png" alt="shape" className="img-fluid" />
                    </div>
                    <div className="offer-icon">
                      {/* <img src="images/industries/healthcare.svg" alt="" className="img-fluid"> */}
                      <svg
                        fill="#000000"
                        version="1.1"
                        id="Capa_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 434.721 434.721"
                        xmlSpace="preserve"
                      >
                        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                          <g>
                            <g>
                              <path d="M126.485,149.111v65.93c0,18.387,14.959,33.348,33.346,33.348h33.347v-65.418h48.366v65.418h33.348 c18.387,0,33.346-14.961,33.346-33.348v-65.928c6.006-1.392,11.519-4.438,15.987-8.904c13-13.001,13-34.158,0-47.159 L240.941,9.768C234.643,3.468,226.27,0,217.361,0c-8.908,0-17.282,3.468-23.581,9.768L110.498,93.05 c-13,13.001-13,34.158,0.002,47.159C114.965,144.676,120.479,147.719,126.485,149.111z"></path>
                              <path d="M184.849,370.971c-1.589-3.889-3.367-7.736-5.286-11.441c-12.563-24.272-29.815-40.471-46.504-56.137 c-4.702-4.412-10.692-11.316-17.034-18.625c-9.258-10.666-18.832-21.695-25.265-25.902c-5.829-3.81-10.592-3.916-14.16-0.312 c-5.202,5.257-3.667,15.048,4.978,31.75c5.777,11.163,14.154,24.312,23.024,38.231c1.425,2.236,2.872,4.51,4.329,6.805 c-15.709-17.182-27.246-32.807-34.344-46.52c-8.152-15.748-9.811-27.918-4.667-34.262c3.632-4.48,10.545-5.595,19.465-3.135 l1.322,0.363l-0.319-1.336c-0.165-0.69-16.648-69.36-20.13-82.357c-0.766-2.866-1.834-5.63-3.171-8.213 c-4.801-9.275-12.388-14.906-18.042-13.39c-2.729,0.733-7.223,3.835-6.631,16.398l8.583,108.402 c0.44,3.539,0.749,7.246,1.077,11.176c1.001,12.01,2.038,24.43,6.953,33.923c2.044,3.951,4.413,7.521,7.24,10.907 c5.09,6.102,19.25,20.834,34.242,36.432c14.593,15.182,29.684,30.883,35.195,37.438c4.323,5.145,8.064,10.814,11.44,17.336 c2.719,5.252,5,10.674,7.038,16.217h52.745l-0.792-2.693C199.801,410.492,192.836,390.521,184.849,370.971z"></path>
                              <path d="M385.677,146.492c-5.654-1.517-13.241,4.114-18.042,13.39c-1.338,2.583-2.405,5.347-3.172,8.213 c-3.48,12.997-19.965,81.667-20.13,82.358l-0.318,1.335l1.321-0.363c8.92-2.46,15.833-1.347,19.466,3.136 c5.144,6.344,3.484,18.512-4.667,34.26c-7.1,13.713-18.636,29.338-34.345,46.521c1.458-2.295,2.904-4.567,4.329-6.806 c8.87-13.92,17.247-27.067,23.024-38.23c8.645-16.701,10.18-26.494,4.978-31.75c-3.567-3.604-8.332-3.498-14.16,0.312 c-6.433,4.207-16.007,15.236-25.265,25.902c-6.342,7.309-12.332,14.211-17.034,18.625c-16.688,15.666-33.941,31.862-46.504,56.135 c-1.919,3.705-3.696,7.555-5.286,11.443c-7.987,19.549-14.952,39.521-21.288,61.055l-0.792,2.693h52.745 c2.038-5.543,4.319-10.963,7.038-16.215c3.376-6.521,7.117-12.191,11.44-17.338c5.511-6.555,20.603-22.256,35.194-37.438 c14.992-15.597,29.151-30.33,34.242-36.431c2.827-3.388,5.196-6.956,7.24-10.907c4.915-9.494,5.952-21.912,6.953-33.923 c0.329-3.93,0.637-7.637,1.077-11.176l8.583-108.402C392.9,150.327,388.405,147.226,385.677,146.492z"></path>
                            </g>
                          </g>
                        </g>
                      </svg>
                    </div>
                    <div className="space24"></div>
                    <a href="javascript:void(0)" className="content-area">
                      <p>Insurance</p>
                      <div className="space18"></div>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>      
      {/*===== INDUSTRIES AREA ENDS =======*/}
    </>
  );
}
